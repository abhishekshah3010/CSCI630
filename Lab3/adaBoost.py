...
def adaDataCollection(exampleFile, hypothesisFile):
    ...
    :param exampleFile:Training file for training
    :param hypothesisFile:Hypothesis file to write the set of hypothesis
    :return:None
    """
    ...
    for hypo in range(totalDecisionStumps):
        ...
        # Updated values for hypothesis weight
...
def returnDecisionStump(depth, rootNode, features, languageLabel, indexOfExamples, weights):
    ...
    for featureIndex in range(len(features)):
        ...
        if enTrue == 0:
            trueValuePending = 0
            falseValuePending = (allFalse / allResult) * calculateEntropy(
                enFalse / allFalse)

        elif enFalse == 0:
            falseValuePending = 0
            trueValuePending = (allTrue / allResult) * calculateEntropy(
                enTrue / allTrue)
